...
/**
 * The main component for the Golf Swing Analysis Demo application.
 * 
 * @component
 ...
 * 
 * @description
 * This component handles the loading and processing of video and JSON files for golf swing analysis.
 * It provides controls for uploading files, toggling guidelines and skeleton visibility, adjusting playback speed, and navigating through analysis steps.
 * 
 * @state {string|null} videoFile - The URL of the uploaded video file.
 * @state {Object|null} jsonData - The parsed JSON data from the uploaded file.
 * @state {number} frameIndex - The current frame index of the video.
 * @state {Object} videoInfo - Information about the video, including fps, width, and height.
 * @state {Object|null} defaulpointGuidelines - Default point guidelines for the analysis.
 * @state {boolean} isPaused - Indicates whether the video is paused.
 * @state {boolean} showGuidelines - Indicates whether to show guidelines on the video.
 * @state {boolean} showSkeleton - Indicates whether to show the skeleton on the video.
 * @state {boolean} isFullScreen - Indicates whether the video is in full-screen mode.
 * @state {number} speed - The playback speed of the video.
 * @state {Object} FrameEnd - The frame indices for the end of various guidelines.
 * @state {Object} defaultdegreeGuidelines - Default degree guidelines for the analysis.
 * @state {Object|null} selectedProblem - The currently selected problem for guideline rendering.
 * @state {number|null} activeStepId - The ID of the currently active step.
 * @state {number} currentStep - The ID of the current step in the analysis.
 * 
 * @function handleVideoUpload - Handles the video file upload and sets the video file URL.
 * @function handleJsonUpload - Handles the JSON file upload, parses the data, and sets the JSON data state.
 * @function handleVideoMetadata - Handles the video metadata load event and updates the video information.
 * @function processFrame - Processes the current frame of the video and updates the frame index and current step.
 * @function startProcessing - Starts the video playback and frame processing.
 * @function pauseProcessing - Pauses the video playback and frame processing.
 * @function handleSeekVideo - Handles the video seek event and updates the frame index.
 * @function mapToScene - Maps a point from the JSON data to the video scene coordinates.
 * @function renderSkeleton - Renders the skeleton on the video based on the JSON data.
 * @function renderGuidelines - Renders the guidelines on the video based on the JSON data and selected problem.
 * @function handleFullScreenChange - Handles the full-screen change event and updates the video information.
 * @function handleEndedVideo - Handles the video ended event and restarts the video from the first step.
 * @function handleStepButtonClick - Handles the step button click event and navigates to the selected step in the analysis.
 */
const App = () => {
  ...
  const handleJsonUpload = (e) => {
    ...
    if (file) {
      reader.onload = (event) => {
        ...
        if (data.Analysis && data.Analysis.Problems && data.Analysis.Problems.length > 0) {
          console.log("data.Analysis.Problems", data.Analysis.Problems);
  ...
  const handleStepButtonClick = (stepId) => {
    ...
    if (jsonData && videoRef.current) {
      const step = jsonData.Analysis.Steps.find(s => s.Id === stepId);
      if (step) {
        const frameTime = step.FrameIndex / videoInfo.fps;
        videoRef.current.currentTime = frameTime;
        setCurrentStep(stepId); // Update currentStep
        startProcessing();
      }
    }
  };
    ...
    <div className="video-analyzer">
      ...
      <div className="file-inputs">
        ...
        <div className="problem-select">
          ...
          <select
            ...
            className="select"
            onChange={(e) => {
              const problemChoice = jsonData.Analysis.Problems.find(
                (problem) => problem.IssueName === e.target.value
              );
              if (problemChoice) {
                setSelectedProblem(problemChoice);
                handleEndedVideo();
                console.log("problemChoice", problemChoice);
              }
            }}
          >
      ...
      <div className="step-buttons-container">
        <div className="step-buttons">
          {GOLF_SWING_STEPS.slice(1).map((step) => (
            <button
              key={step.id}
